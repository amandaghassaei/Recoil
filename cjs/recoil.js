...
function mergeDependencyMapIntoGraph(deps, graph) {
...
function getRecoilValueAsLoadable(
  store,
  {key},
  ...
  if (typeof valueOrUpdater === 'function') {
    ...
    const state =
      ...
      _storeState$nextTree !== void 0
        ...
        : storeState.currentTree; // NOTE: This will evaluate node, but not update state with node subscriptions!
    ...
    store.replaceState(
      Recoil_Tracing.wrap(state => {
        ...
        return {
          ...
          nonvalidatedAtoms: mapByDeletingMultipleFromMap$1(
            state.nonvalidatedAtoms,
            writtenNodes,
          ),
        };
...
function markRecoilValueModified(store, {key}) {
  Recoil_Tracing.trace('mark RecoilValue modified', key, () =>
    store.replaceState(
      Recoil_Tracing.wrap(state => ({
        ...state,
...
function recoilValuesForKeys(keys) {
  return Recoil_mapIterable(keys, key =>
...
class Snapshot {
  constructor(storeState) {
    ...
    _defineProperty(this, 'getPromise', recoilValue =>
      Recoil_gkx_1('recoil_async_selector_refactor')
    ...
    _defineProperty(this, 'getSubscribers_UNSTABLE', ({key}) => {
      const state = this._store.getState().currentTree;

      const downstreamNodes = Recoil_filterIterable(
        getDownstreamNodes$1(this._store, state, new Set([key])),
        nodeKey => nodeKey !== key,
      );
      return {
...
function cloneSnapshot(store, version = 'current') {
  ...
  const treeState =
    version === 'current'
      ...
      : Recoil_nullthrows(storeState.previousTree);
...
const {useContext, useEffect, useMemo, useRef, useState} = react; // @fb-only: const RecoilusagelogEvent = require('RecoilusagelogEvent');
...
function RecoilRoot({
  ...
  const subscribeToTransactions = (callback, key) => {
    if (key == null) {
      ...
      const id = nextID++;
      Recoil_nullthrows(nodeTransactionSubscriptions.get(key)).set(
        id,
        callback,
      );
      return {
...
function useRecoilValueLoadable_MUTABLESOURCE(recoilValue) {
  ...
  return getRecoilValueAsLoadable$2(storeRef.current, recoilValue, treeState);
}
...
const useMutableSource =
  (_useMutableSource = react.useMutableSource) !== null &&
  _useMutableSource !== void 0
    ? _useMutableSource
    : react.unstable_useMutableSource; // flowlint-line unclear-type:off
...
function useRecoilSnapshot() {
  ...
  const [snapshot, setSnapshot] = useState$1(() =>
    cloneSnapshot$1(storeRef.current),
  );
  useTransactionSubscription(
    useCallback(store => setSnapshot(cloneSnapshot$1(store)), []),
  );
  return snapshot;
}
    ...
    snapshot => {
      ...
      reactDom.unstable_batchedUpdates(() => {
        ...
        for (const keys of [prev.atomValues.keys(), next.atomValues.keys()]) {
          for (const key of keys) {
            ...
            if (
              ...
              getNode$2(key).shouldRestoreFromSnapshots
            ) {
        ...
        storeRef.current.replaceState(state => {
          return {...state, stateID: snapshot.getID_INTERNAL()};
        });
      });
    },
    [storeRef],
...
function selector(options) {
  ...
  function evaluateSelectorFunction(store, state) {
    ...
    try {
      ...
      if (!isP) {
        ...
        loadable = loadableWithPromise$1(
          errorOrDepPromise
            .then(() => {
  ...
  function myPeek(store, state) {
    ...
    const depValues = new Map(
      Array.from(currentDeps)
        .sort()
        .map(depKey => [depKey, peekNodeLoadable$2(store, state, depKey)]),
    );
...
function atom(options) {
  ...
  if (
    isRecoilValue$4(optionsDefault) // Continue to use atomWithFallback for promise defaults for scoped atoms
...
var Recoil_index_3 = Recoil_index.AppContext;
...
exports.AppContext = Recoil_index_3;
